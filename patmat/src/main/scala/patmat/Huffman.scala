...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      if (chars == Nil) Nil
        ...
        val head = chars.head
        val count = chars.count(_ == head)
        (head, count) :: times(chars.filter(_ != head))
      }
    }
   ...
   */
    def codeBits(table: CodeTable)(char: Char): List[Bit] = {
      table.find(_._1 == char).map(_._2).getOrElse(Nil)
    }
   ...
   */
    def convert(tree: CodeTree): CodeTable = tree match {
      ...
      case Fork(l, r, _, _) => {
        val leftTable = convert(l).map { case (c, bits) => (c, 0 :: bits) }
        val rightTable = convert(r).map { case (c, bits) => (c, 1 :: bits) }
