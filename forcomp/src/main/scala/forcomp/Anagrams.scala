...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (ch, n) :: rest =>
      ...
      (for {
        i <- 0 to n
        comb <- restCombinations
      } yield if (i == 0) comb else (ch, i) :: comb).toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x.map { case (ch, n) => (ch, n - yMap.getOrElse(ch, 0)) }
      .filter { case (_, n) => n > 0 }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(occ: Occurrences): List[Sentence] = occ match {
      ...
      case _ =>
        for {
          comb <- combinations(occ) if dictionaryByOccurrences.contains(comb)
          word <- dictionaryByOccurrences(comb)
          rest <- iter(subtract(occ, comb))
        } yield word :: rest
    }
